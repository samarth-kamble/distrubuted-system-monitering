...
export function AdminAuditLogsView() {
  ...
  const totalPages = Math.ceil(dbAuditLogs.length / itemsPerPage);
  const paginatedLogs = dbAuditLogs.slice(
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between pt-4 border-t border-border/40 mt-4">
                <span className="text-[11px] text-muted-foreground font-mono">
                  Showing {((currentPage - 1) * itemsPerPage) + 1} to {Math.min(currentPage * itemsPerPage, dbAuditLogs.length)} of {dbAuditLogs.length} logs
                </span>
