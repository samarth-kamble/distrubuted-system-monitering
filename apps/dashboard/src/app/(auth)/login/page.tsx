...
export default function LoginPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    setIsLoading(true)
    setTimeout(() => setIsLoading(false), 2000) // Simulated redirect
  }
  ...
  return (
    <div className="w-full max-w-105 space-y-8">
      ...
      <div className="grid grid-cols-2 gap-4">
        <button className="flex h-11 items-center justify-center gap-2 rounded-lg border border-border bg-card px-4 text-sm font-medium shadow-2xs hover:bg-muted/50 transition-all cursor-pointer">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <Button type="submit" disabled={isLoading} className="w-full font-bold cursor-pointer h-11 mt-2 shadow-sm">
          {isLoading ? (
            <>
