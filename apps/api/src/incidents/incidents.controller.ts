...
export class IncidentsController {
  ...
  findOne(@Request() req: RequestWithUser, @Param('id') id: string) {
    return this.incidentsService.findOne(req.user.id, req.user.tenantId, req.user.role, id);
  }
  ...
  acknowledge(@Request() req: RequestWithUser, @Param('id') id: string) {
    return this.incidentsService.acknowledge(req.user.id, req.user.tenantId, req.user.role, id);
  }
  ...
  resolve(@Request() req: RequestWithUser, @Param('id') id: string) {
    return this.incidentsService.resolve(req.user.id, req.user.tenantId, req.user.role, id);
  }
}
